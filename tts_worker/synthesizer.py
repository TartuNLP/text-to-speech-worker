...
class Synthesizer:
    def __init__(self, model_config: ModelConfig, max_input_length: int = 0):
        ...
        self.vocoder = HIFIGAN.from_hparams(source="speechbrain/tts-hifigan-libritts-22050Hz")
    ...
    def _synthesize(self, text: str, speaker: str, speed: float = 1) -> (Response, int):
        ...
        for i, sentence in enumerate(sentences):
            ...
            while True:
                ...
                try:
                    sent_durations = []
                    if len(normalized_sentence) > max_input_length:
                        inputs = split_sentence(normalized_sentence, max_len=max_input_length)
                    ...
                    for input_sentence in inputs:
                        ...
                        if mel_spec.size:  # don't send empty mel-spectrograms to vocoder
                            waveform = self.vocoder.decode_batch(torch.tensor(mel_spec).unsqueeze(0)).squeeze()
                    ...
                    logger.warning(
                        ...
                        f"trying again with {int(min((last_input_len, max_input_length)) * 0.5)}")
        ...
        out = io.BytesIO()
        wavfile.write(out, self.sampling_rate, waveform.astype(np.float32))
